...
if ($step == 10) {
	...
	print Log "host gene\n"; my $time=time();
	print Log "########################################\n";
	my $job="perl $Bin/bin/step10.host.pl -out $outdir -dsh $outdir/work_sh -queue $queue";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
...
if ($step == 12) {
	print Log "########################################\n";
	print Log "get host gene fasta\n"; my $time=time();
	print Log "########################################\n";
	my $job="perl $Bin/bin/step12.fa.pl -out $outdir -dsh $outdir/work_sh -queue $queue";
	print Log "$job\n";
	`$job`;
	print Log "$job\tdone!\n";
	print Log "########################################\n";
...
Script:		$Script
Add: 
Description:	Framework by long.huang
	            modified from beibei.ma
				maintain By Meng Luo
Usage:
	...
	#-soft	<string> bwa && CIRI2 |  bowtie && find_circ  | STAR && CIRCexplorer  | TopHat-Fusion && CIRCexplorer ##
	-soft	<string> CIRI2(bwa) |  find_circ (bwa) | CIRCexplorer(bwa) | circ_finder (STAR), default \"all\"
	...
	-species <str> required  Animals-a or Plants-p 
	-h			Help
